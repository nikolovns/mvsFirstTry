<?php

namespace Repository;

use Models\UserInfoModel;

class UserInfo {

    /**
     *
     * @var \Database\Db
     */
    protected $db;

    /**
     *
     * @var \Repository\UserInfo
     */
    protected static $inst;

    protected function __construct(\Database\Db $db) {
        $this->db = $db;
    }

    /**
     * @return UserInfo
     */
    public static function createInstance() {
        if (self::$inst == null) {
            self::$inst = new self(\Database\Db::getInstance());
        }
        return self::$inst;
    }



    public function selectAllById($id) {
        $query = 'SELECT gender, first_name, last_name, email
                    FROM user_info WHERE user_id = ?';
        $this->db->query($query, [$id]);

        $result = $this->db->row();

        return new \Models\UserInfoModel(
            $result['gender'],
            $result['first_name'],
            $result['last_name'],
            $result['email']
        );

    }

    public function selectAllConferencesDetailsByUserId($id) {
        $query = 'SELECT c.confName, c.startDate, c.endDate, c.venue_id, c.id, v.name, v.address
                  FROM conferences c
                  JOIN venues v
                  ON v.id = c.venue_id
                  WHERE ';

        $this->db->query($query);

        $result = $this->db->fetchAll();

        $allConferences = [];

        foreach ($result as $key => $value) {
            $allConferences[] = new \BindingModels\Conference(
                $value['confName'],
                $value['startDate'],
                $value['endDate'],
                $value['venue_id'],
                $value['id'],
                $value['name'],
                $value['address']
            );
        }
        return $allConferences;
    }

    public function save(UserInfoModel $user) {

        $query ='INSERT INTO user_info(user_id, gender, first_name, last_name, email)
                VALUE (?, ?, ?, ?, ?)';

        $param = [
            $user->getUserId(),
            $user->getGender(),
            $user->getFirstName(),
            $user->getLastName(),
            $user->getEmail()
        ];

        $this->db->query($query, $param);
        return $this->db->row();
    }


}